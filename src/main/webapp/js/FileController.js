 ...
 * @constructor
 */
var FileController = function($scope, $http, $window) {
	$scope.isSave=true;
	$scope.isShow = false;
    ...
    $scope.fetchFilesList = function() {
        $http.get('cloudDisk.do').success(function(FileList){
...
//        console.log(save_button);
    ...
    $scope.removeFile = function() {
    	...
    	if (typeof($scope.deleteFile)!="undefined" && $scope.deleteFile!=null)
    	{
    		...
    		for(var i in $scope.Files){
    			var obj = $("#check_box_"+$scope.Files[i].id);
    			if(obj.prop('checked')){
    				$scope.selectedFiles.push($scope.Files[i]);
    			}	
    		}
    		console.log($scope.selectedFiles);
    		$http.post('cloudDisk/removeFiles.do',$scope.selectedFiles).success(function(){
    			  $scope.fetchFilesList();
    		})
    	}
    };
